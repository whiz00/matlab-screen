...
function! matlab#RunSelected() range
    ...
    call jobsend(s:matlab_buf,"cd ".expand('%:p:h').";matlab_tmp\n")
endfunction

function! matlab#RunCurrentFile()
    call jobsend(s:matlab_buf,"cd ".expand('%:p:h').";".
...
function! matlab#GetDoc()
    call jobsend(s:matlab_buf,"doc ".expand("<cword>")."\n")
endfunction

function! matlab#SetBreak()
    call jobsend(s:matlab_buf, "dbstop in ".expand('%:p'." at ".line(".")."\n"))
endfunction

function! matlab#WatchVarible()
    let var_name = substitute(getline('.'),'^.*[a-zA-Z0-9._]\@<!\(\S*\%'.col('.').'c\k*\).*$','\1', '')
    call jobsend(s:matlab_buf, "openvar ".var_name."\n")
endfunction

function! matlab#OpenCurrentFile()
    call jobsend(s:matlab_buf, "edit ".expand('%:p')."\n")
endfunction
...
function! matlab#OpenAllFile()
    ...
    while currentBuffer <= bufnr('$')
        ...
        let currentBuffer = currentBuffer + 1
    endwhile
    call jobsend(s:matlab_buf, "edit ".allFileStr."\n")
endfunction

function! matlab#OpenWorkspace()
    call jobsend(s:matlab_buf, "workspace\n")
endfunction

function! matlab#ClearVariables()
    call jobsend(s:matlab_buf, "clear\n")
endfunction

function! matlab#GetVariableSize()
    call jobsend(s:matlab_buf, "size(" . expand("<cword>") . ")\n")
endfunction
